...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-2026.6.11-orange.svg" alt="Version">
</p>
...
- **Local Docker Compose deployment** — single-host stack, all services bring up via `./install.sh && docker compose up -d`
- **Baseline workflow** — manual one-time historical load (additive `edgeguard baseline` or destructive `edgeguard fresh-baseline`)
- **Incremental sync workflow** — Airflow DAGs (`edgeguard_pipeline`, `edgeguard_medium_freq`, `edgeguard_low_freq`, `edgeguard_daily`, `edgeguard_neo4j_sync`) running on cron schedules
- **11 collectors** (OTX, NVD, CISA KEV, MITRE ATT&CK, VirusTotal, AbuseIPDB, ThreatFox, URLhaus, CyberCure, Feodo, SSL Blacklist) → MISP → Neo4j
- **REST API** (FastAPI, port 8000) + **GraphQL API** (Strawberry, port 4001 — ISIM-compatible)
- **Real-time NATS alerting** for new high-confidence IOCs
- **Auth** (`EDGEGUARD_API_KEY`, auto-generated by `install.sh`) + safety check refusing unauthenticated 0.0.0.0 binds
- **Healthchecks + Prometheus + Grafana monitoring stack** (local Docker Desktop / Mac scrape via `host.docker.internal`)
...
- **Local operational hardening** — see [Issue #53](../../issues/53) (PR-E backlog) and post-merge audit follow-ups:
  ...
  - Per-DAG staleness alerts for the daily/3-day sync DAGs
  - `MISPToNeo4jSync.run(incremental=True)` direct test coverage
  - `update_source_checkpoint` concurrency-safety tests
...
EdgeGuard v2026.6.11 is **production-test ready**. Full pipeline validated on Docker (Mac Mini); unit tests + lint + type-check + BugBot green (Trivy on the apache/airflow base image and the pip-audit starlette advisory are known-red — tracked in issues #52/#121/#124).
...
- **MISP as Single Source of Truth**: All collectors write to MISP first with full provenance tagging; OTX_META/TF_META/NVD_META comment prefixes for metadata round-trip; `alias:<name>` tags round-trip actor & malware aliases into Neo4j (PR #126)
