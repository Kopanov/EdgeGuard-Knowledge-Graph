...
(:Vulnerability {
  ...
  tag: STRING,              // Source tag (part of UNIQUE constraint with cve_id)
...
(:Indicator {
  ...
  tag: STRING,              // Source tag (part of UNIQUE constraint)
  sources: LIST,            // Accumulated source list (deduped)
...
(:Malware {
  ...
  tag: STRING,              // Source tag (part of UNIQUE constraint with name)
  sources: LIST,            // Accumulated source list (deduped)
...
CREATE CONSTRAINT cvssv2_key FOR (cv2:CVSSv2) REQUIRE (cv2.vector_string, cv2.tag) IS UNIQUE;
CREATE CONSTRAINT cvssv30_key FOR (cv30:CVSSv30) REQUIRE (cv30.vector_string, cv30.tag) IS UNIQUE;
CREATE CONSTRAINT cvssv31_key FOR (cv31:CVSSv31) REQUIRE (cv31.vector_string, cv31.tag) IS UNIQUE;
CREATE CONSTRAINT cvssv40_key FOR (cv40:CVSSv40) REQUIRE (cv40.vector_string, cv40.tag) IS UNIQUE;
