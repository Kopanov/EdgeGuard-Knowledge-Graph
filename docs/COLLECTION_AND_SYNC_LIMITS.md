...
| **`EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE`** | **`run_misp_to_neo4j.sync_to_neo4j()`** (MISP → Neo4j) | Max **parsed graph items** merged **per Python chunk** into Neo4j (controls **worker RAM** during write). Default **`500`**. **`0`** / **`all`** = single pass (OOM risk). | Environment on Airflow worker |
| **`EDGEGUARD_REL_BATCH_SIZE`** | **`run_misp_to_neo4j._create_relationships()`** → **`Neo4jClient.create_misp_relationships_batch()`** | Max **relationship definitions** per Neo4j **UNWIND** batch. Default **`500`**. | Environment on Airflow worker |
| **MISP "EdgeGuard" event discovery** | **`run_misp_to_neo4j.fetch_edgeguard_events()`** | **Primary:** paginated **`GET /events/index`** (then **`/events`**) — lightweight rows **without** scanning all attributes (avoids **`restSearch`** timeouts on huge events). **Client-side filter:** **`Event.info`** contains **`EDGEGUARD_MISP_EVENT_SEARCH`** (default **`EdgeGuard`**) **or** **`org.name`** is **`EdgeGuard`**. Incremental runs also filter by **`timestamp`** / **`date`** when present. **Fallback (PR-N29 H3):** PyMISP / **`POST /events/restSearch`** with **`search`** + **paginated** `_FALLBACK_PAGE_SIZE=500` × `_FALLBACK_MAX_PAGES=200` (= 100K events safety cap). Cap-hit raises `_MispFallbackHardError` rather than silently truncating (PR-N29 audit HIGH-2). Pre-PR-N29 the fallback used a single `limit: 1000` request with no pagination — silent data-loss risk on populated MISP. Primary index env knobs: **`EDGEGUARD_MISP_PAGE_SIZE`** (default **500**), **`EDGEGUARD_MISP_MAX_PAGES`** (default **100**). | Code in **`src/run_misp_to_neo4j.py`** |
...
2. **MISP → Neo4j sync** (`run_misp_to_neo4j`, Airflow **`run_neo4j_sync`** / **`full_neo4j_sync`**)  
   ...
   **Chunk size** for Neo4j **node** writes is **`EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE`** (default **500** items, 3s pause between chunks). **Relationship** batches use **`EDGEGUARD_REL_BATCH_SIZE`** (default **500** definitions).
...
- **“I set `BASELINE_COLLECTION_LIMIT=1000`, so MISP→Neo4j only loads 1000 things.”**  
   **No.** Baseline limit caps **per-source collector** pushes (OTX, NVD, …). Sync discovers events via **paginated index** (then optional **paginated restSearch** fallback per PR-N29 — `_FALLBACK_PAGE_SIZE=500` × `_FALLBACK_MAX_PAGES=200`, raises `_MispFallbackHardError` on cap-hit) — unrelated to baseline Variables.
...
_Last updated: 2026-04-26 — PR-N33 docs audit: corrected MISP fetch fallback semantics (was pre-PR-N29 single-shot `limit: 1000`, now post-PR-N29 paginated `_FALLBACK_PAGE_SIZE=500 × _FALLBACK_MAX_PAGES=200` with `_MispFallbackHardError` on cap-hit); exposed `EDGEGUARD_MISP_PAGE_SIZE` / `EDGEGUARD_MISP_MAX_PAGES` env-knob names; removed unverified "3-second pause between chunks" claim from `EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE` row. Prior: 2026-04-06._
