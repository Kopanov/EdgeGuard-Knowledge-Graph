...
| **`EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE`** | **`run_misp_to_neo4j.sync_to_neo4j()`** (MISP → Neo4j) | Max **parsed graph items** merged **per Python chunk** into Neo4j (controls **worker RAM** during write). Default **`500`**. **`0`** / **`all`** = single pass (OOM risk). 3-second pause between chunks. | Environment on Airflow worker |
