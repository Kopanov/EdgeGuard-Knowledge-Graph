...
8. **OOM / worker killed during Neo4j insert:** The sync merges in **Python-side chunks** (default **500** items, 2s pause). Events with **>5000 attributes** are streamed in pages of 5000. Default **`AIRFLOW_MEMORY_LIMIT`** is **12g** (100K+ attr events need 8-12GB). Lower **`EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE`** if still OOM.
