...
2. **`run_neo4j_sync`** — imports **`MISPToNeo4jSync`**, chooses **full** vs **incremental** sync (first-ever run or Airflow Variable **`NEO4J_FULL_SYNC=true`** → full; otherwise incremental, default window last 3 days), calls **`sync.run()`**, then on success updates the state file and metrics.
   - **OOM on very large attribute sets:** `sync_to_neo4j()` merges in **Python-side chunks** (default **1000** items per chunk, sorted so tactics/techniques/malware/actors precede vulns/indicators). Set **`EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE`** (e.g. `500`) on the Airflow worker if the process is killed during Neo4j writes. **`0`** or **`all`** (case-insensitive) disables Python chunking — **one pass over the full sorted list** (same memory profile as pre-chunking code; **OOM risk** on huge backfills; expert / large-RAM / debugging only — logs a warning above 2k / 5k items). `Neo4jClient.merge_*_batch` still uses its own UNWIND batch size (default 1000).
...
**Limits (don’t confuse these):** Baseline **`BASELINE_COLLECTION_LIMIT`** caps **external collectors** (OTX, NVD, …), **not** how many MISP events the sync reads. The sync’s event list uses a **hardcoded search `limit: 1000`** for “EdgeGuard” events (**no pagination** yet). **`EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE`** (default **1000**) controls **Neo4j merge RAM** (parsed items per Python chunk), **not** MISP fetch size. See **[COLLECTION_AND_SYNC_LIMITS.md](COLLECTION_AND_SYNC_LIMITS.md)**.
