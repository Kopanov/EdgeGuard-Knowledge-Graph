...
5. **MISP→Neo4j sync RAM**: For very large MISP backfills, the sync uses **Python-side chunked merges** (default **1000** items per chunk via **`EDGEGUARD_NEO4J_SYNC_CHUNK_SIZE`**). Lower the value if the worker is OOM-killed or gets **SIGKILL (-9)** during Neo4j writes. With Docker Compose, the **airflow** service has a **memory limit** (**`AIRFLOW_MEMORY_LIMIT`**, default **4g** in `docker-compose.yml`) — raising it is often the fix when the host still has free RAM. **`0`** or **`all`** forces a **single pass** (OOM risk). **`THREATFOX_API_KEY`** (Abuse.ch) must be set in the same env as Airflow if you want ThreatFox collection to run instead of skipping / 401.
